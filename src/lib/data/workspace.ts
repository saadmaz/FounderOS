...
export async function ensureWorkspaceForUser(user: User): Promise<string> {
  ...
  const member: Omit<WorkspaceMember, "id"> = omitUndefined({
  ...
  // The member doc's create rule verifies ownership via get() on the
  // workspace doc, and Firestore rules evaluate get() against the snapshot
  // from *before* the current commit - a single atomic batch could never
  // see this workspace's own doc, so the workspace must land first.
